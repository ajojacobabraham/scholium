...
export default function AccountsPage() {
  // 2. Select the raw accounts array and the getter function separately
  ...
  // 3. Memoize the tree calculation so it only runs when 'accounts' changes
    ...
    <div className="max-w-3xl mx-auto">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 text-slate-400">
          ...
          <p className="text-sm mt-1">Create your first knowledge or goal account to get started</p>
        </div>
      ) : (
        <div className="bg-white rounded-xl border border-slate-200 divide-y divide-slate-100">
          {tree.map((node) => (
            <AccountTreeItem key={node.id} node={node} depth={0} />
          ))}
        </div>
      )}
